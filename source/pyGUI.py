...
from PyQt5.QtGui import QFont, QPalette, QStandardItem, QFontMetrics
...
class GUI(QMainWindow):
    ...
    def quitGUI(self):
        ...
        # self.acquireDataProcess.terminate()

    def sendBoardSetting(self):
...
def startGUI(dataDict, boardBuffer, boardApiCallEvents, _share, _newDataAvailable, _isConnected):
    app = QApplication(sys.argv)
    try:
        # board = boardBuffer.get()
        board = boardBuffer
    except Exception:
        board = None
        print("No board found in the buffer")

    gui = GUI(dataDict, board, boardApiCallEvents, _share, _newDataAvailable, _isConnected)
    gui.show()
    sys.exit(app.exec_())
