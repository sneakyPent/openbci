...
def set_checkerboards_position(window):
	...
	checkerboards_position[0] = [-(videoW+distance*2+checker_s*checker_num)/2, (window.size[1]-checker_s*checker_num)/3]  # left
	checkerboards_position[1] = [(videoW+distance*2+checker_s*checker_num)/2 , (window.size[1]-checker_s*checker_num)/3]  # right
	checkerboards_position[2] = [-(videoW+distance*2+checker_s*checker_num)/2, -(window.size[1]-checker_s*checker_num)/3]  # back
	checkerboards_position[3] = [(videoW+distance*2+checker_s*checker_num)/2, -(window.size[1]-checker_s*checker_num)/3]  # forward
...
def set_arrows_position(checkerboards_position):
	...
	arrows_position[0] = [int(round(checkerboards_position[0][0])), int(round(checkerboards_position[0][1]/2))]  # left
	arrows_position[1] = [int(round(checkerboards_position[1][0])), int(round(checkerboards_position[1][1]/2))]  # right
	arrows_position[2] = [int(round(checkerboards_position[2][0])), int(round(checkerboards_position[2][1]/2))]  # back
	arrows_position[3] = [int(round(checkerboards_position[3][0])), int(round(checkerboards_position[3][1]/2))]  # forward
...
def arrow_init(window, position):
	...
	triangle_up = visual.ShapeStim(win=window, name='triangle_up', units='pix',
									   vertices=[[0, arrow_s], [arrow_s/2, -arrow_s], [0, -arrow_s/2], [-arrow_s/2, -arrow_s]],
	...
	rect_center = visual.ShapeStim(win=window, name='rect_center', units='pix',
								   vertices=[[rect_s, rect_s], [rect_s, -rect_s], [-rect_s, -rect_s], [-rect_s, rect_s]],
								   ori=0, pos=[0, 0],
								   lineWidth=1, lineColor=[255, 255, 128], lineColorSpace='rgb255',
								   fillColor=[255, 255, 128], fillColorSpace='rgb255',
								   opacity=1, depth=-2.0,
...
def SSVEP_screen_session(board, startPresentation, boardApiCallEvents, _shutdownEvent, currentClassBuffer, target_dur, frames_ch, mode):
		...
		if startPresentation.is_set():
			...
			try:
				...
				for target_ind in shuffled_targets:
					...
					while continueRoutine and t < (0.0 + (target_dur-win.monitorFramePeriod*0.75)) and startPresentation.is_set():
						# get current time
						t = flash_checkerboardClock.getTime()
						...
						if thisTarget.status == STARTED and t >= (0.0 + (1.0-win.monitorFramePeriod*0.75)): #most of one frame period left
...
def SSVEP_online_SCREEN_session(board, startPresentation, boardApiCallEvents, _isReading, _shutdownEvent, currentClassBuffer, groundTruthBuffer, frames_ch, _streaming,
				...
				while continueRoutine and startPresentation.is_set():
					...
					if end_fl.status == STARTED:
						...
						# add the commands for the ground truth
							...
							groundTruthBuffer.put(cnst.groundTruthKeyboardCommands_class4Switcher[cnst._keyboardKey_FORWARD])
							currentClassBuffer.put_nowait(300)
							print("GROUND TRUTH SET FORWARD")
						elif cnst.groundTruthKeyboardCommands[cnst._keyboardKey_RIGHT] in theseKeys:
							groundTruthBuffer.put(cnst.groundTruthKeyboardCommands_class4Switcher[cnst._keyboardKey_RIGHT])
							currentClassBuffer.put_nowait(300)
							print("GROUND TRUTH SET RIGHT")
						elif cnst.groundTruthKeyboardCommands[cnst._keyboardKey_LEFT] in theseKeys:
							groundTruthBuffer.put(cnst.groundTruthKeyboardCommands_class4Switcher[cnst._keyboardKey_LEFT])
							currentClassBuffer.put_nowait(300)
							print("GROUND TRUTH SET LEFT")
						elif cnst.groundTruthKeyboardCommands[cnst._keyboardKey_BACK] in theseKeys:
							groundTruthBuffer.put(cnst.groundTruthKeyboardCommands_class4Switcher[cnst._keyboardKey_BACK])
							currentClassBuffer.put_nowait(300)
							print("GROUND TRUTH SET BACK")
						elif cnst.groundTruthKeyboardCommands[cnst._keyboardKey_STOP] in theseKeys:
							groundTruthBuffer.put(cnst.groundTruthKeyboardCommands_class4Switcher[cnst._keyboardKey_STOP])
							currentClassBuffer.put_nowait(300)
